...
a-tabs.tabs(defaultActiveKey='1')
  a-tab-pane.tab(tab='Ответственный' key='1')
    a-select.select(size='large' :defaultValue='responsible'
      ...
      a-select-option(v-for='(user, i) in filteredUsers' :key='`user${i}`' :value='user.user.id')
        .inner-opt
  ...
  a-tab-pane.tab(tab='Соисполнители' key='2')
    a-select.select(size='large' :defaultValue='coresponsibles' mode="multiple"
                    @change='changeCoresponsibles' :disabled='!isAssignor')
      a-select-option(v-for='(user, i) in filteredUsers' :key='`user${i}`' :value='user.user.id')
        .inner-opt
          a-avatar.ava(v-if='getAvatar(user.user)' :size='32' :src='getAvatar(user.user)')
          a-avatar.ava(v-else icon="user")
  ...
  a-tab-pane.tab(tab='Наблюдатели' key='3')
    a-select.select(size='large' :defaultValue='observers' mode="multiple"
                    @change='changeObservers' :disabled='!isAssignor')
      a-select-option(v-for='(user, i) in filteredUsers' :key='`user${i}`' :value='user.user.id')
        .inner-opt
          a-avatar.ava(v-if='getAvatar(user.user)' :size='32' :src='getAvatar(user.user)')
          a-avatar.ava(v-else icon="user")
  ...
  a-tab-pane.tab(tab='Постановщик' key='4')
    a-select.select(size='large' :defaultValue='assignor'
                    @change='changeAssignor' :disabled='!isAssignor')
      a-select-option(v-for='(user, i) in filteredUsers' :key='`user${i}`' :value='user.user.id')
        .inner-opt
          a-avatar.ava(v-if='getAvatar(user.user)' :size='32' :src='getAvatar(user.user)')
          a-avatar.ava(v-else icon="user")
...
export default {
  ...
  computed: {
    isAssignor() {
      if (this.users && this.users.filter((u) => u.role == 'admin').length > 0
          && this.users.filter((u) => u.role == 'admin')[0].user.id == this.$store.state.user.id) {
        return true;
      }
      if (this.$store.state.role == 'admin') {
        return true;
      }
      if (this.assignor && this.assignor[0] == this.$store.state.user.id) {
        return true;
      }
      return false;
    },
