    ...
    a-row(:gutter="16", v-if='!isLoading&&project!=null')
      ...
      a-col(:span='9')
        a-card(:title='project.name', :bodyStyle=' {maxHeight: "100%"} ')
            ...
            a-col
              a-select(:disabled='isDisabled'
                ...
                a-select-option(v-for='member in project.members' :key='member.user.id'
                  ...
                  .inner-opt
                    a-avatar.ava(:size='24', v-if='getAvatar(member.user)'
          ...
          a-row
            a-col
              a-select(:filterOption='filterMember' :disabled='isDisabled'
                      :defaultValue='getGuests' mode="multiple"
                      @change="handleChangeGuests" @select='addObserver'
                      placeholder="Наблюдатели" style='width: 100%')
          ...
          a-row
            a-col
              a-select(:filterOption='filterMember' :disabled='isDisabled'
                      :defaultValue='getMembers'
                      @change="handleChangeMembers" @select='addMember'
                      @deselect='removeMember' mode="multiple"
                      placeholder="Участники" style='width: 100%')
                a-select-option(v-for='user in getFilteredUsers'
                                :key='`${user.first_name} ${user.last_name} ${user.id}`'
                                :value='user.id')
                  .inner-opt
